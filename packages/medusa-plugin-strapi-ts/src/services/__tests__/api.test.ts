	...
	beforeAll(async () => {
		...
		if (typeResult) {
			...
			if (result) {
				...
				if (result) {
					result = await service.createProductVariantInStrapi('exists', defaultAuthInterface);
					if (result) {
						app = mockServer();
						...
						container.register(
							'productVariantService',
							asFunction(() => service.productVariantService_).singleton()
						);
						...
						container.register(
							...
							asFunction(() =>
								jest
									.fn()
									.mockReturnValue(Promise.resolve(MockRepository('productCollectionRepository')))
							).singleton()
						);
						container.register(
							'storeService',
