...
async function testPost(testInfo) {
  ...
  const apiEndpoint = isSingleType
    ? testInfo.schema.info.singularName
    : testInfo.schema.info.pluralName;
  ...
  if (!isMedia) {
    ...
    if (!isSingleType) {
      postResult = await request(strapi.server.httpServer)
        ...
        .attach(
          `files.${mediaFields[0]}`,
          `${__dirname}/./fixtures/coffee-art.jpg`
        )
        .field(
          "data",
          '{ "medusa_id": "1", "filename": "coffee-art.jpg","name":"test","title":"test" }',
          {
  ...
  const id =
    ...
    postResults[apiEndpoint]?.data?.slug;
...
async function testGet(apiName, id, isSingleType) {
  ...
  if (!id) {
    ...
    getResult = await request(strapi.server.httpServer)
      .get(url)
      .set("Authorization", `Bearer ${creds.body.jwt}`);
  ...
  for (const apiSingular of apisGlobalVar) {
    it(`sanity check /get ${apiSingular}`, async () => {
      ...
      const apiEndPoint = isSingleType
        ...
        : testInfo.schema.info.pluralName;
      if (!isSingleType) {
        sanityCheckResult[apiEndPoint] = await testGet(
          apiEndPoint,
          undefined,
          isSingleType
        );
    ...
    for (const apiSingular of apiTestList) {
      ...
      it(`testing  ${apiSingular}`, async () => {
        ...
        if (postResults[apiEndpoint]) {
          ...
          putResult[apiEndpoint] = await testPut(
            apiEndpoint,
            id,
          ...
          deleteResult[apiEndpoint] = await testDelete(
            apiEndpoint,
            id,
            isSingleType
          );
