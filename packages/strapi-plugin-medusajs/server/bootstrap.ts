...
export async function createUserWithAdminRole(params: RoleParams, role: { id: number }): Promise<any> {
...
async function createDefaultAdminRoles(): Promise<void> {
	try {
		...
		if (!superAdminRole) {
			...
			strapi.log.warn('Successfully created super user admin role, author admin role and editor admin roles');
		}
	} catch (e) {
