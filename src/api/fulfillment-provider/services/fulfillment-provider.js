    ...
    try {
      if (data && data.length) {
        for (const fulfillmentProvider of data) {
          ...
          const found = await strapi.services['api::fulfillment-provider.fulfillment-provider'].findOne({ medusa_id: fulfillmentProvider.medusa_id });
if (found) {
  continue
}
...
return true;
  ...
  try {
    for (const fulfillmentProvider of fulfillmentProviders) {
      ...
      const found = await strapi.services['api::fulfillment-provider.fulfillment-provider'].findOne({
