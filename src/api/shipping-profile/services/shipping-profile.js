    ...
    try {
      if (data && data.length) {
        for (const shipping_profile of data) {
          ...
          const found = await strapi.services['api::shipping-profile.shipping-profile'].findOne({ medusa_id: shipping_profile.medusa_id });
if (found) {
  continue;
}
...
return true;
  ...
  const found = await strapi.services['api::shipping-profile.shipping-profile'].findOne({ medusa_id: shippingProfile.medusa_id });
  if (found) {
    return found.id;
  }
